...
vignette: >
  %\VignetteIndexEntry{Overview of the earthR Package}
...
The earthR package provides a range of functions that operate on (clean), and visualize,
...
The raw data is provided by NOAA as a compressed, tab-separated variable file.  We begin by reading that file into a data frame named data_raw.  The raw data frame is then processed by eq_clean_data() and the cleaned data frame is summarized as shown below:

```{r}
library(earthR)
# First read in the raw data file into a data frame from its compressed tab-separated variable file
data_raw <- readr::read_delim("signif.tsv", delim = "\t")
# After we have the raw data, clean it up for use by the visualization functions
eq_data <- eq_clean_data(data_raw)
